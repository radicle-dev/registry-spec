...
\[
    \tx{identify}{\field{I}{pk}, \field{I}{sig}}
\]
...
\spec{
    ...
    \begin{itemize}
        ...
        \item $\field{I}{sig}$ is a signature verifying that the transaction
            author owns $\field{I}{pk}$.
    \end{itemize}
    \validation
    \begin{itemize}
        \item $\field{I}{pk}$ is not already associated with an account,
        \item $\field{I}{sig}$ is $\alpha_{id}$ signed by the secret key $sk$ that
            $\field{I}{pk}$ was derived from. In other words,
            \[
                \field{I}{sig} = \mathsf{encrypt(hash(\alpha_{id}), \mathit{sk})}
            \]
            which is valid if
            \[
                \mathsf{decrypt}(\field{I}{sig}, \field{I}{pk}) \equiv
