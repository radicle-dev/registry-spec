...
\spec{
    ...
    \begin{itemize}
        ...
        \item $\field{P}{checkpoint}$ is the $id$ of the latest project
            checkpoint, or $\varnothing$ if the project hasn't been
            checkpointed yet (See \S\ref{s:checkpoint}),
...
\spec{
    ...
    \begin{itemize}
        \item $\field{P}{id}$ refers to a project that has been accepted
            in the registry,
        \item $k'$ refers to a checkpoint which links back to the same initial
            checkpoint as the previous project checkpoint $k$.
    \end{itemize}
    \outputs
    \begin{itemize}
        \item $\field{P}{checkpoint} = k'$
    \end{itemize}}
...
% TODO
% - Transferring project names
% - Changing project names
% - Project name update pointer - should verify that initial/root checkpoint is same
% - Funds being locked with associated keys
% - Refreshing names
% - De-associate a public key with your account
% - Talk about forks in checkpoint chains
% - Project contract updating
% - Does registration include a contract, eg. if there are multiple owners?
% - What can be verified off-chain and flagged?
% - Can we compress the initial checkpoint to not submit unsigned commits, or _all_ commits?
% - Perhaps we can use a Merkle proof and root hash in a checkpoint to compress things?
% - Re-organize document to have transactions under the big sections, eg. accounts
