    ...
    \begin{itemize}
        ...
        \item $\field{P}{contract}$ is the project contract, which governs
            ...
            \[
                f : T \to \{\top, \bot\} \cup \nat.
            \]
            where $T$ is any verified transaction.  An output of $\top$
            signifies a transaction $t \in T$ is \emph{authorized}, while
            $\bot$ means \emph{unauthorized}. An output of type $\nat$ is
            reserved for transactions that should trigger a transfer of
            funds. In that case $\nat$ is the set of valid values to transfer.
